  ...
  run(callback) {
    ...
    function generateText(err) {
      ...
      async.each(_.keys(self.tables), function(table, _callback) {
        ...
        text[table] += "//////////////////////////////////////////////////////////////////////\n";
        text[table] += "//\n";
        text[table] += "//  table name: " + table + "\n";
        text[table] += "// create time: " + (moment().format('YYYY-MM-DD HH:mm:ss')) + "\n";
        //text[table] += "//      author: " + (moment().format('YYYY-MM-DD HH:mm:ss')) + "\n";
        text[table] += "//\n";
        text[table] += "//////////////////////////////////////////////////////////////////////\n";
        text[table] += "\n";
        ...
        text[table] += spaces + "const sequelize = app.Sequelize;\n\n";
        ...
        _.each(fields, function(field, i) {
          ...
          //console.log('self.tables[table]=', self.tables[table][field]);
          // column's attributes
          ...
          _.each(fieldAttr, function(attr, x) {
            ...
            if (attr === "foreignKey") {
              ...
              //console.log('table =', table, 'field =', field, 'attr =', attr);
              var _attr = (self.tables[table][field][attr] || '').toLowerCase();
              //console.log('---------');
              var _val = self.tables[table][field][attr];
              var val = quoteWrapper + _val + quoteWrapper;
              if (attr === 'comment') {
                if (_val === null){
                  val = '\'\'';
                } else {
